...
트랙 A 12개 태스크가 전부 끝났고 main에 머지·배포까지 완료했다. PR #1~#9.

배포: https://diamondscout-ai.onrender.com (동작 확인함)

UI는 여러 번 바꿨다가 최종적으로 초기 위저드로 되돌렸다.
- 랜딩 → 시작하기 → 투수/타자 탭 → STEP 1~4 → 분석 실행
- 존 표시만 오늘 만든 캔버스 씬(실사 인물 + 3D 존 + 코스 미리보기)으로 교체
- 중간에 만들었던 한 화면 콘솔과 3단계 분리는 되돌렸다. 코드는 히스토리에 있다
  (콘솔 = 2effb52 이전, 3단계 = cb63670)

예측 성능(서빙 실측): top-1 43.84% / top-3 86.38% / ECE 0.0081
테스트 121건 통과.

## 남은 것

- 목표 top-1 47%까지 3.2%p. 남은 후보는 LightGBM 하이퍼파라미터 정도인데
  `best_iteration=89`가 이례적으로 이른 수렴이라 여지는 있다. 다만 하이퍼파라미터로
  3%p가 나오는 경우는 드물어서, 47%가 이 피처 세트로 닿는 수치인지부터 다시 볼 필요가 있다.
- 데모 영상 없음. Render 무료 티어는 15분 무활동 시 슬립해서 첫 로딩에 80초쯤 걸린다.
  제출용으로 짧은 영상이 있으면 안전하다.
- `docs/TROUBLESHOOTING_SUMMARY.md` 신설(10건 한 장 요약). 루트 `TROUBLESHOOTING.md`가
  전체 기록이다.
...
| | |
|---|---|
| TS-010 | GRU 학습 교착 해결. 원인은 pyarrow와 TF의 dylib 로드 순서 |
| Task 7·8 | GRU 학습 완주 + 앙상블 게이트 통과 (w=0.30) |
| Task 9 | 온도 스케일링 캘리브레이션. ECE 0.0126 → 0.0081 |
| Task 11 | 배포 아티팩트 2티어 통합. 38MB 제거, sklearn·joblib 배포 의존성에서 제외 |
| Task 12 | ADR-0006 작성, ADR-0001 Superseded 처리. 정확도 표를 동일 split으로 교체 |
| — | 타자 피처 누수 수정(train 경기만 집계) |
| — | 타자 x 구종 피처 측정 후 미채택 |
| — | ADR-0007(Render 배포) 작성 |
| — | UI 되돌리기 + 존을 캔버스 씬으로 교체 |
...
| 9 확률 캘리브레이션 | 완료 — ECE 0.0126 -> 0.0081, 서빙 반영 | PR #2 |
| 11 배포 아티팩트 + Render 검증 | 코드·검증 완료 / 머지했는데 재배포가 안 됨 | `bef4ddd` |
| 12 문서 + ADR | 완료 — ADR-0006으로 작성 | `41221f4` |

12개 태스크 전부 완료. Task 11 Step 5(실제 배포 확인)만 막혀 있다.
...
계획서는 "ADR-0003"으로 쓰라고 돼 있는데 그 번호는 이미 "UI 프레임워크로 Gradio 채택"이
쓰고 있다. 0005까지 차 있어서 ADR-0006으로 썼다. 계획서가 옛 상태 기준이었던 것 같다.

- `docs/ADR.md`에 ADR-0006(피처 보강 + LightGBM + GRU 앙상블) 추가. ADR-0001은
  Superseded로 표시했다. ADR-0001이 남긴 후속 과제 두 개(같은 테스트 셋으로 재비교,
  TensorFlow를 프로덕션에 들일지)가 여기서 처리됐다.
- `docs/PERFORMANCE.md` 정확도 표를 같은 test split, 같은 표본(88,983건)으로 교체.
  최빈 구종 고정 베이스라인(FF 32.7% / 61.1%)을 실측해 맨 위에 넣었다. LSTM은
  10,000건으로만 재서 표에서 빼고 각주로 돌렸다.
- `README.md`의 모델명을 LightGBM + GRU로 고쳤다.

문서를 훑다가 사실과 다른 것 두 개를 발견해서 같이 고쳤다.

- README가 "별도 배포 없이 Gradio share=True 터널로만 공개"라고 적고 있었다. 실제로는
  Render에 올라가 있고 같은 문서 위쪽이 Render 주소를 링크한다.
- ADR-0005(share=True 터널 배포)도 같은 이유로 낡았다. Superseded로 표시하고 TS-006과
  render.yaml을 가리키게 했다. Render 전환 자체를 다루는 ADR은 아직 없다 - 쓸지는
  판단이 필요하다.

## 완료: Task 9 확률 캘리브레이션

화면에 확률을 그대로 띄우므로 그 숫자가 실제 빈도와 맞아야 한다. 온도 스케일링으로
test ECE를 0.0126에서 0.0081로 낮췄다. 순위를 안 바꾸는 방식이라 top-1/top-3는 그대로다.

계획서와 다르게 간 지점이 하나 있다. 계획서는 NLL을 최소화해 T를 찾으라고 했는데
그렇게 하면 T=0.85가 나오고 test ECE가 0.0126에서 0.0299로 오히려 나빠진다. 모델이
이미 거의 맞춰져 있어서(ECE 1.26%) NLL을 더 낮추려면 확률을 뾰족하게 만들어야 하고,
그러면 확신도가 정확도를 앞지른다. val ECE를 직접 최소화하는 격자 탐색으로 바꿔
T=0.95를 골랐다. 두 목적함수의 결과를 스크립트가 둘 다 출력한다.

`services/prediction_service.py`가 `models/calibration.json`을 읽어 앙상블 확률에
건다. 파일이 없으면 T=1.0으로 아무것도 안 한다.

## 막힘: Render가 머지 후 재배포되지 않는다

PR #1을 2026-08-18 04:07Z에 main으로 머지했는데, 140분이 지나도 배포된 앱이 구버전이다.
PR #2(캘리브레이션)까지 머지한 뒤에도 같다.

확인한 것:
- 사이트는 살아 있다. HTTP 200이 나온다.
- `/config`의 첫 컴포넌트가 `# 이모지 DiamondScout AI` 마크다운이다. 지금 `app.py`는
  `gr.HTML(HEADER_HTML)`로 시작하므로 구 위저드 UI가 떠 있는 것이다.
- `ds-console`, `ds-card` 같은 새 UI 클래스가 응답에 없다.
- keep-warm 워크플로는 계속 성공하고 있다. 서버 자체는 정상이다.

140분이면 이 의존성 구성(gradio, lightgbm, pandas, matplotlib)의 빌드 시간을 한참
넘긴다. "아직 빌드 중"은 아니다. Auto-Deploy가 꺼져 있거나 서비스가 이 저장소의
main을 안 보고 있을 가능성이 크다. 저장소만 봐서는 확정할 수 없다.

사람이 Render 대시보드에서 확인해야 하는 것:
1. Auto-Deploy가 켜져 있는지
2. 서비스가 보고 있는 브랜치가 `main`이 맞는지
3. `render.yaml`을 Blueprint로 적용해 만든 서비스인지, 대시보드에서 직접 만든
   서비스인지. 후자면 `render.yaml` 수정이 반영되지 않는다. 그 경우 대시보드에
   `PITCH_MODEL_FILE` 환경변수가 남아 있을 텐데, 지금 코드는 그 변수를 안 읽으므로
   해가 되지는 않는다. 지워두면 깔끔하다.
4. 빌드 로그에 실패가 있는지

수동 배포로 한 번 밀어보면 1번과 4번이 바로 갈린다.

배포가 뜨면 볼 것: 분석 1회 실행, Render 로그에 `Ran out of memory`가 없는지.
로컬(macOS) 실측은 app.py 전체 349MB였다.
