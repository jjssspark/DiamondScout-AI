...
def test_clamp_step_moves_within_range():
    from ui.console import clamp_step

    assert clamp_step(1, +1) == 2
    assert clamp_step(3, -1) == 2


def test_clamp_step_stops_at_the_ends():
    """첫 단계에서 이전, 마지막에서 다음을 눌러도 죽지 않고 그 자리에 선다."""
    from ui.console import clamp_step

    assert clamp_step(1, -1) == 1
    assert clamp_step(3, +1) == 3
    assert clamp_step(99) == 3
    assert clamp_step(0) == 1


def test_step_visibility_shows_exactly_one_block():
    from ui.console import STEP_LABELS, step_visibility

    for step in range(1, len(STEP_LABELS) + 1):
        flags = step_visibility(step)
        assert len(flags) == len(STEP_LABELS)
        assert sum(flags) == 1
        assert flags[step - 1] is True


def test_step_bar_marks_done_now_and_next():
    from ui.console import render_step_bar

    bar = render_step_bar(2)

    assert bar.count("ds-stepdot--done") == 1
    assert bar.count("ds-stepdot--now") == 1
    assert bar.count("ds-stepdot--next") == 1
    assert 'aria-current="step"' in bar


def test_step_bar_labels_every_step():
    from ui.console import STEP_LABELS, render_step_bar

    bar = render_step_bar(1)
